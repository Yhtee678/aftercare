...
export function DictationSummary({ task, today, unresolved }: { task: {
  ...
  return <div className="space-y-2">
    <p className="break-words text-sm text-slate-600">{task.schoolName ? `${task.schoolName} · ${task.academicYear} · Grade ${task.grade} · ${task.className}` : "Individual dictation"}</p>
    ...
    <p className="text-sm text-slate-600">Assigned {task.assignedDate} · Scheduled {task.scheduledDate}</p>
