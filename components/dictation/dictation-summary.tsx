...
export function DictationSummary({ task, today, unresolved }: { task: {
  contentFormat?: string; source: keyof typeof dictationSourceLabels; type: keyof typeof dictationTypeLabels; description: string;
  ...
  return <div className="space-y-2">
    <p className="break-words text-sm text-slate-600">{task.schoolName ? `${task.schoolName} · ${task.className}` : "个人听写"}</p>
    <h2 className="font-semibold">{dictationSourceLabels[task.source]} · {dictationTypeLabels[task.type]}</h2>
    <DictationContent text={task.description} format={task.contentFormat} />
