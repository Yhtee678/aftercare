...
import { DictationPhoto } from "./dictation-photo";
...
export function StudentDictationRow({ assignment, context, number, hidden = false }: { number: number; hidden?: boolean; context: string; assignment: { id: string; studentName: string; status: DictationStatus } }) {
  ...
  return <li hidden={hidden} className={`space-y-3 rounded-xl border p-4 ${completed ? "border-slate-200 bg-slate-50 text-slate-600" : correction ? "border-amber-300 bg-amber-50" : "border-slate-200 bg-white"}`}>
    ...
    <DictationPhoto context={`${assignment.studentName}\n${context}`} />
