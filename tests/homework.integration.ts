...
async function run() {
  ...
  try {
    ...
    assert.ok(correctionHtml.includes("需要订正") && correctionHtml.includes("1 未完成"));
    ...
    assert.ok((await html(`/homework/${id}`)).includes("0 未完成"));
    ...
    assert.ok(!otherClassPage.includes(input.description) && otherClassPage.includes("此班级暂无功课"));
    assert.ok((await html(`/homework/${id}`)).includes(`/homework/classes/${classIds[0]}`));
    for (const invalid of ["bad", randomUUID()]) assert.ok((await html(`/homework/classes/${invalid}`)).includes("找不到班级"));
    assert.ok((await html("/homework/new")).includes(`value="${classIds[0]}"`));
    for (const invalid of ["bad", randomUUID()]) assert.ok((await html(`/homework/${invalid}`)).includes("找不到功课"));
