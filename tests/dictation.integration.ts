...
async function run() {
  ...
  try {
    ...
    assert.ok(correctionHtml.includes("Needs Practice") && correctionHtml.includes("1 unresolved"));
    ...
    assert.ok((await html(`/dictation/${id}`)).includes("0 unresolved"));
    ...
    for (const invalid of ["bad", randomUUID()]) assert.ok((await html(`/dictation/classes/${invalid}`)).includes("Class not found"));
    assert.ok((await html("/dictation/new")).includes(`value="${classIds[0]}"`));
    for (const invalid of ["bad", randomUUID()]) assert.ok((await html(`/dictation/${invalid}`)).includes("Dictation not found"));
